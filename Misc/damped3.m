...
% Step 11: Test how changing the values of npt, nst, and nmax impacts the results

% Define test values for npt, nst, and nmax
npt_values = [101, 201, 301]; % Different spatial discretization points
nst_values = [13, 50, 100];    % Different time sampling points
nmax_values = [3, 5, 10];      % Different truncation levels

% Loop over different values of npt, nst, and nmax
%for np = npt_values
%    for ns = nst_values
%        for nm = nmax_values
%            % Recompute the parameters based on the current settings
%            x = linspace(0, L, np)';   % Update x based on npt
%            t_movie = linspace(0, 4 * pi / omega_n(1), ns); % Update time array based on nst
%
%            % Compute initial conditions f(x) and g(x)
%            f_x = zeros(np, 1);
%            g_x = zeros(np, 1); % g_x is still zeros for this case
%
%            % Recalculate initial conditions (Pinches)
%            for i = 1:np
%                if x(i) <= L/2
%                    f_x(i) = x(i) / (L/2);
%                else
...
%            end
%
%            % Recalculate eigenfunctions
%            phi_n = zeros(np, nm); % Initialize array for eigenfunctions
%            for n = 1:nm
%                phi_n(:, n) = sqrt(2/L) * sin(n * pi * x / L);
%            end
%            
%            % Continue with the rest of the computations...
%            % For instance, calculate frequencies, periods, etc.
%
%            % Visualization can be added here to compare results
%            % For example, plot f(x) and its approximation based on the chosen nmax
%            
%            % This code will be run for each combination of parameters
%        end
%    end
%end
