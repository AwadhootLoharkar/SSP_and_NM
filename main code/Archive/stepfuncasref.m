...
%% Step 1: Define the Potential Profile (Step Function)
% Define the step function U0(x < 0) = 0 and U0(x > 0) = U1 = -0.1 eV
U1 = -0.1;  % Step potential in eV
U0 = zeros(size(x)); % Initialize potential
U0(x > 0) = U1;      % Step at x > 0

% Define the additional potential profile across the heterostructure
% Modify the potential profile according to equation (7.95)
x_min = 0; 
x_max = 15; 
E_bias = abs(U1) / (x_max - x_min); % Electric field associated with bias
U = U0; % Initialize the final potential
U(x >= x_min & x <= x_max) = U(x >= x_min & x <= x_max) - E_bias * x(x >= x_min & x <= x_max);
...
xlabel('Position [Å]');
ylabel('Potential [eV]');
title('Potential Profile with Step Function');
grid on;
...
for i = 1:length(E_range)
    ...
    G = zeros(N, N);
    % Calculate the Green's function G1 for the step function potential
    for m = 1:N
        for n = 1:N
...
Density_of_States = (1 / (pi * hbar)) * (gamma ./ ((E0 - E_range).^2 + gamma^2));
...
title('Density of States for the Step Function Potential');
grid on;

%% Step 5: Tuning the Bias Potential
E0 = 0.01; % Electron at Fermi level
...
for j = 1:length(U1_values)
    ...
    U_tuned(x > 0) = U1_values(j); % Apply new bias
